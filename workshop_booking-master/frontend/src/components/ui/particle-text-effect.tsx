...
const DEFAULT_WORDS = ["WELCOME", "IN", "FOSSEE", "WORKSHOP"]
let auroraColorIndex = 0

export function ParticleTextEffect({ words = DEFAULT_WORDS, theme = 'random' }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const newColor = theme === 'aurora'
      ? (() => {
          const c = AURORA_COLORS[auroraColorIndex % AURORA_COLORS.length]
          auroraColorIndex++
          return c
        })()
      : {
  ...
  const animate = () => {
    ...
    // Background with motion blur (User's specific request)
